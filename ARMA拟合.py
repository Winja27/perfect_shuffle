...
def visualize_shuffle_cycles(max_m):
    ...
    # Fit the data using an ARMA model
    model = ARIMA(cycle_lengths, order=(2, 0, 2))  # Example order, can be tuned
    model_fit = model.fit()
    fitted_cycle_lengths = model_fit.fittedvalues

    # Plot fitted curve
    plt.plot(m_values, fitted_cycle_lengths, '-', label='Fitted ARMA Model')
    plt.title('Cycle Lengths for Perfect Shuffles with Fitted ARMA Model')
